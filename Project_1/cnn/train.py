...
class Network(tf.keras.Model):
    def __init__(self):
        ...
        self.conv1 = tf.keras.layers.Conv2D(24, 5, 1, padding='same', activation=tf.nn.relu, kernel_regularizer=regularization_func(0.7))  # 卷积层一
        self.pool1 = tf.keras.layers.MaxPool2D(2, 2) # 池化层一
        self.conv2 = tf.keras.layers.Conv2D(32, 3, (1, 2), padding='same', activation=tf.nn.relu, kernel_regularizer=regularization_func(0.7))  # 卷积层二
        self.pool2 = tf.keras.layers.MaxPool2D(2, 2) #池化层二
        self.conv3 = tf.keras.layers.Conv2D(48, 3, 1, padding='same', activation=tf.nn.relu, kernel_regularizer=regularization_func(0.7))
